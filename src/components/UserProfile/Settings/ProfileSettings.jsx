...
import { FaSearch, FaQrcode, FaUser, FaComments, FaBell, FaQuestionCircle, FaDatabase, FaUserPlus } from 'react-icons/fa';
...
const settingsOptions = [
  ...
  { icon: <FaDatabase />, label: 'Storage and data', description: 'Network usage, storage usage', path: '/dashboard/userProfile/settings/storage' },
  { icon: <FaUserPlus />, label: 'Invite a friend', description: '', path: '/dashboard/userProfile/settings/invite' },
];
const users = [{name: 'Nazrul Islam', src:Profile ,description:"Never give up 💪"}];
      ...
      <div className="bg-white rounded-t-3xl min-h-screen p-6 h-96 mt-4">
       { users.map((user)=>(
          ...
          <div className="w-16 h-16 rounded-full overflow-hidden">
            <img src={user.src} alt="" className="w-full h-full object-cover" />
          </div>
          <div className="flex-1 ml-6">
            <h3 className="text-black flex-1 font-bold text-lg">{user.name}</h3>
